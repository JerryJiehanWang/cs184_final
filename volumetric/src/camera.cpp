...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //
    //std::cout<< x << ", " << y <<endl;
    double x_cam = x * 2 * tan(radians(hFov) * 0.5) - tan(radians(hFov)*0.5);
    double y_cam = y * 2 * tan(radians(vFov) * 0.5) - tan(radians(vFov)*0.5) ;
    Vector3D dir = c2w * Vector3D(x_cam, y_cam, -1);
    
    //std::cout<< dir.unit();
    Ray r = Ray(this->pos, dir.unit());
    r.max_t = this->fClip;
    r.min_t = this->nClip;
    return r;
